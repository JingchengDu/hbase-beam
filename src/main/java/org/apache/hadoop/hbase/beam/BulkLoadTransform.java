...
public class BulkLoadTransform<I> extends PTransform<PCollection<I>, PDone> {

  private SerializableConfiguration conf;
  private String tableName;
  private SerializableBytesArray startKeys;
  private MakeFunction<I, Cell> cellFunc;
  ...
  public BulkLoadTransform(SerializableConfiguration conf, String tableName, URI stagingPathUri,
      URI tmpPathUri, SerializableBytesArray startKeys, boolean sorted,
      MakeFunction<I, Cell> cellFunc) {
    this(conf, tableName, stagingPathUri, tmpPathUri, startKeys, sorted, cellFunc, false);
  }

  public BulkLoadTransform(SerializableConfiguration conf, String tableName, URI stagingPathUri,
      URI tmpPathUri, SerializableBytesArray startKeys, boolean sorted,
      MakeFunction<I, Cell> cellFunc, boolean minorCompactionExclude) {
    this.conf = conf;
  ...
  public PDone expand(PCollection<I> inputs) {
    ...
    PCollection<KV<byte[], Cell>> cells = map(inputs);
    // 2. Run GroupByKey
    PCollection<KV<byte[], Iterable<Cell>>> cellsPartitions = group(cells);
    // 3. Sort and output in each group
    foreachGroup(cellsPartitions);
    return PDone.in(inputs.getPipeline());
  }

  private PCollection<KV<byte[], Iterable<Cell>>> group(PCollection<KV<byte[], Cell>> cells) {
    return cells.apply("groupByKey" + UUID.randomUUID().toString(),
      GroupByKey.<byte[], Cell> create());
  }

  private void foreachGroup(PCollection<KV<byte[], Iterable<Cell>>> cellsPartitions) { 
    cellsPartitions.apply(ParDo.of(new DoFn<KV<byte[], Iterable<Cell>>, Void>() {
      ...
      public void processElement(ProcessContext c) throws Exception {
        ...
        Iterable<Cell> cells = c.element().getValue();
        if (cells != null) {
          Cell lastCell = null;
          for (Cell cell : cells) {
            if (cell != null) {
  ...
  private PCollection<KV<byte[], Cell>> map(PCollection<I> inputs) {
    return inputs.apply("prepareKV" + UUID.randomUUID().toString(),
      ParDo.of(new DoFn<I, KV<byte[], Cell>>() {
        @ProcessElement
        public void processElement(ProcessContext c) throws Exception {
          Cell cell = cellFunc.make(c.element());
          if (cell != null) {
            byte[] startKey = startKeys.get()[partitionFor(cell)];
            c.outputWithTimestamp(KV.of(startKey, cell), c.timestamp());
          }
        }
      }));
  }
  ...
  protected int partitionFor(Cell cell) {
    int index = Bytes.binarySearch(startKeys.get(), cell.getRowArray(), cell.getRowOffset(),
      cell.getRowLength(), Bytes.BYTES_RAWCOMPARATOR);
    if (index >= 0) {
      return index;
    } else {
      return -1 * index - 2;
